...
import { BrowserRouter, Route, Link, Switch } from './my-react-router';
...
//     evt.preventDefault();
//     evt.stopPropagation();
//     window.history.pushState({did: this.props.detailId}, "详情页", this.props.href);
//   }

//   render() {
//     return(
//       <a onClick={this.click} href={this.props.href}>{this.props.children}</a>
//     );
//   }
// }
...
class Main extends Component {
  ...
  getList() {
    return fetch("http://localhost:9000/list").then(res => res.json()).then(data => {
      return {
        type: 'PUSH_LIST',
        data: data.data
      }
  ...
  componentDidMount() {
    this.props.updateList(this.getList);
  }
              ...
              <React.Fragment>
                ...
                <List
                  ...
                  renderItem={(item, idx) => {
                    ...
                    return <ItemComponent key={idx} {...item} onClick={() => { console.log('跳转'); }} />
                  }}
                />
              </React.Fragment>
          }
